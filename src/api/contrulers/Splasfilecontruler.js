...
const SplasFile = require("../../models/Splasscreen");
const fs = require('fs');
...
exports.fileadd = async (req, res, next) => {
  upload.any()(req, res, async (err) => {
    ...
    try {
      ...
      const savedFiles = await SplasFile.insertMany(files);
      res.json(savedFiles);
    } catch (err) {
...
exports.filesget = async (req, res) => {
  try {
    const files = await SplasFile.find();
...
exports.filedelet = async (req, res) => {
  ...
  try {
    const file = await SplasFile.findByIdAndDelete(fileId);
