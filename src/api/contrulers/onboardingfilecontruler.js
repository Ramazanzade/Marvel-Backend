...
const Onboardingfile = require("../../models/Onboarding");
const fs = require('fs');
...
exports.fileadd = async (req, res, next) => {
  upload.any()(req, res, async (err) => {
    ...
    try {
      ...
      const savedFiles = await Onboardingfile.insertMany(files);
      res.json(savedFiles);
    } catch (err) {
...
exports.filesget = async (req, res) => {
  try {
    const files = await Onboardingfile.find();
...
exports.filedelet = async (req, res) => {
  ...
  try {
    const file = await Onboardingfile.findByIdAndDelete(fileId);
